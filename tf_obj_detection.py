...
#
# --- models --- object_detection --- tf-obj_detection.py
#                                  |
...
# Some Errors
#  - Using pyplot (matplotlib) installed with pip on your virtual environment in your MacOS. 
...
See the detection model zoo for a list of other models that can be run out-of-the-box 
...
Here we use internal utility functions, but anything that returns a dictionary mapping integers to 
...
'''
OpenCV
Instead of using the sample images, you can use your webcam by chaning a couple of lines in the code. 
'''
  ...
  with tf.Session(graph=detection_graph) as sess:
    while True: ###
      ...
      vis_util.visualize_boxes_and_labels_on_image_array(
          ...
          use_normalized_coordinates=True,
          line_thickness=8)
      cv2.imshow('Window Name', cv2.resize(image_np,(480, 320))) ###
